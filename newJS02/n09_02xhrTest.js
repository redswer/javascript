...
=> 클라이언트로부터 요청 이벤트 발생시 실행되는 메서드:startMethod() 작성 
    -> 생성된  XMLHttpRequest 객체 를 이용해서 요청준비 (결과처리를 담당할 메서드를 지정) -> 요청 설정 -> 보냄
3. 결과처리
=> 결과처리 담당 메서드 작성
=> 응답 결과가 오면 자동 실행
 */
...
// XMLHttpRequest 객체의 인스턴스를 보관할 변수를 전역 선언
let xhrObj;
...
// 3. 
function resultF() {
    // readyState == 4 : 요청객체의 상태가 모든 데이터를 받을 수 있는 상태로 완료된 경우
    if (xhrObj.readyState == 4) {

        // status == 200 : response 성공
        if (xhrObj.status == 200) {
            document.getElementById('resultArea').innerHTML = '서버 Response :<br>' + xhrObj.responseText;

        } else {
            document.getElementById('resultArea').innerHTML = '서버 Response :<br>' + '찾을 수 없음';
        }
    }
}
